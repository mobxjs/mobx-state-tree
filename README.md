...
| [`getSafeReference(() => node | null | undefined, checkIfAlive = true)`](API.md#getsafereference)         | Tests if a reference is valid (pointing to an existing node and optionally if alive) and returns such reference if it the check passes, else it returns undefined.                                                                                    |
