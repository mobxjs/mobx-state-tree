...
-   `.preProcessSnapshot(snapshot => snapshot)` (deprecated in favor of `types.snapshotProcessor`) can be used to pre-process the raw JSON before instantiating a new model. See [Lifecycle hooks](#lifecycle-hooks-for-typesmodel)
-   `.postProcessSnapshot(snapshot => snapshot)` (deprecated in favor of `types.snapshotProcessor`) can be used to post-process the raw JSON before getting a model snapshot. See [Lifecycle hooks](#lifecycle-hooks-for-typesmodel)
...
The exception to this rule are the (deprecated in favor of `types.snapshotProcessor`) `preProcessSnapshot` and `postProcessSnapshot` hooks:
