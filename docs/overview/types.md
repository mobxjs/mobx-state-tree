...
-   [`types.enumeration(name?, options: (string | number)[])`](/API/#enumeration) creates an enumeration. This method is a shorthand for a union of string or number literals, e.g. `types.enumeration("PaymentPeriod", [15, 30])`. If you are using typescript and want to create a type based on a string enum (e.g. `enum Color { ... }`) then use `types.enumeration<Color>("Color", Object.values(Color))`, where the `"Color"` name argument is optional. Note that `Object.values` on a *numeric* TypeScript enum also returns the member names (`["A", "B", 15, 30]`), so list numeric options explicitly instead.
